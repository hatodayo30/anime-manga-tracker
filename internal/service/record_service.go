...
func (s *RecordService) List(ctx context.Context, userID int64, mediaType model.MediaType, status model.Status) ([]*model.Record, error) {
	...
	return s.repo.List(ctx, userID, mediaType, status)
}

// AddOrUpdateStatus は作品をライブラリに追加する。既に登録済みならステータスのみ更新する
// （検索画面でステータスボタンを押し直すケースに対応）。
func (s *RecordService) AddOrUpdateStatus(ctx context.Context, userID int64, in model.NewRecordInput) (*model.Record, error) {
...
func (s *RecordService) UpdateProgressOrStatus(ctx context.Context, userID, id int64, in model.UpdateRecordInput) (*model.Record, error) {
